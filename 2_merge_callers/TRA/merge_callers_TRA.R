...
if(n_rows!=0 & sum(num_callers)>1){
  ...
  # comment multiple lines Highlight the text and use CTRL+SHIFT+C
  
  ## remove breakpoints close to 150bp #####
# 
#   type = "translocation"
# 
#   final_data$ID = "none"
# 
#   final_data$ID[1] = paste0(type,"_",1)
# 
#   for(final_data_var in 2:nrow(final_data)){
# 
#     if(final_data$chr_1[final_data_var]==final_data$chr_1[(final_data_var-1)] &
#        final_data$chr_2[final_data_var]==final_data$chr_2[(final_data_var-1)] &
#        (abs(final_data$start_1[final_data_var]-final_data$start_1[(final_data_var-1)])<=150 |
#         abs(final_data$start_2[final_data_var]-final_data$start_2[(final_data_var-1)])<=150) &
#        final_data$GT[final_data_var]==final_data$GT[(final_data_var-1)]){
# 
#       final_data$ID[final_data_var] = paste0(final_data$ID[(final_data_var-1)])
#     }
#     else{final_data$ID[final_data_var] = paste0(type,"_",final_data_var)}
# 
#   }
# 
#   final_data_ok = data.frame(ID = unique(final_data$ID))
# 
#   final_data_ok$chr_1 = 0
#   final_data_ok$start_1 = 0
#   final_data_ok$end_1 = 0
# 
#   final_data_ok$chr_2 = 0
#   final_data_ok$start_2 = 0
#   final_data_ok$end_2 = 0
# 
#   final_data_ok$GT_pindel = 0
#   final_data_ok$GT_delly = 0
#   final_data_ok$GT_lumpy = 0
#   final_data_ok$GT_manta = 0
#   final_data_ok$GT_svaba = 0
# 
#   final_data_ok$GT = 0
#   final_data_ok$length = 0
# 
# 
#   for(final_data_var in 1:nrow(final_data_ok)){
# 
#     aux = final_data %>% filter(ID==final_data_ok$ID[final_data_var])
# 
#     final_data_ok$chr_1[final_data_var] = unique(aux$chr_1)
#     final_data_ok$start_1[final_data_var] = floor(min(aux$start_1))
#     final_data_ok$end_1[final_data_var] = floor(max(aux$start_1))
# 
#     final_data_ok$chr_2[final_data_var] = unique(aux$chr_2)
#     final_data_ok$start_2[final_data_var] = floor(min(aux$start_2))
#     final_data_ok$end_2[final_data_var] = floor(max(aux$start_2))
# 
#     final_data_ok$GT_pindel[final_data_var] = paste0(unique(aux$GT_pindel),collapse = "")
#     final_data_ok$GT_delly[final_data_var] = paste0(unique(aux$GT_delly),collapse = "")
#     final_data_ok$GT_lumpy[final_data_var] = paste0(unique(aux$GT_lumpy),collapse = "")
#     final_data_ok$GT_manta[final_data_var] = paste0(unique(aux$GT_manta),collapse = "")
#     final_data_ok$GT_svaba[final_data_var] = paste0(unique(aux$GT_svaba),collapse = "")
# 
#     final_data_ok$GT[final_data_var] = paste0(unique(aux$GT),collapse = "")
#     final_data_ok$length[final_data_var] = max(c(final_data_ok$end_2[final_data_var]-final_data_ok$start_2[final_data_var],
#                                              final_data_ok$end_1[final_data_var]-final_data_ok$start_1[final_data_var]))
# 
#   }
# 
#   final_data_ok$GT_pindel[-which(final_data_ok$GT_pindel %in% c("0/1","1/1","NA"))] = "./."
#   final_data_ok$GT_delly[-which(final_data_ok$GT_delly %in% c("0/1","1/1","NA"))] = "./."
#   final_data_ok$GT_lumpy[-which(final_data_ok$GT_lumpy %in% c("0/1","1/1","NA"))] = "./."
#   final_data_ok$GT_manta[-which(final_data_ok$GT_manta %in% c("0/1","1/1","NA"))] = "./."
#   final_data_ok$GT_svaba[-which(final_data_ok$GT_svaba %in% c("0/1","1/1","NA"))] = "./."
# 
#   final_data_ok$GT[-which(final_data_ok$GT %in% c("0/1","1/1"))] = "./."
# 
#   final_data_ok$GT_pindel[final_data_ok$GT_pindel=="NA"] = NA
#   final_data_ok$GT_delly[final_data_ok$GT_delly=="NA"] = NA
#   final_data_ok$GT_lumpy[final_data_ok$GT_lumpy=="NA"] = NA
#   final_data_ok$GT_manta[final_data_ok$GT_manta=="NA"] = NA
#   final_data_ok$GT_svaba[final_data_ok$GT_svaba=="NA"] = NA
# 
    ## end remove breakpoints 150 bp
  ...
  my_pred = caret::predict.train(mod_fit,data_predict) #paquete caret requiere de "predict.train"
