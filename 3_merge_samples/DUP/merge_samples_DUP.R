...
while(n_row==0 & sample_initial<786){
  ...
  sample1 = fread(paste0("/2_merge_callers/DUP/outputs/",ids[sample_initial],"/",ids[sample_initial],"_DUP_",chr),
                  header = T,fill = T)
...
for(sample_merge in sample_initial:785){
    ...
    sample1 = fread(paste0("/2_merge_callers/DUP/outputs/",ids[sample_merge],"/",ids[sample_merge],"_DUP_",chr),
                    header = T,fill = T)
 ...
 dir.create(paste0("/3_merge_samples/DUP/outputs/",chr))
  
  final_merge %>% 
    fwrite(paste0("/3_merge_samples/DUP/outputs/chr_",chr,
                  "/DUP_chr_",chr,"_",as.character(start),"_",as.character(end)),row.names=F,sep=" ",na = "NA")
  
  print("Compressing file")
  
  system(paste0("/3_merge_samples/DUP/outputs/chr_",chr,
                "/DEL_chr_",chr,"_",as.character(start),"_",as.character(end)))
  
  print("Generating VCF")
  
  final_merge = fread(paste0("zcat /3_merge_samples/DUP/outputs/chr_",chr,
  ...
  fwrite(my_vcf[which_miss_05],paste0("/3_merge_samples/DUP/outputs/chr_",chr,
                       "/DUP_chr_",chr,"_",as.character(start),"_",as.character(end),".vcf"),row.names=F,sep="\t")
  
  print("Compressing final VCF")
    
  system(paste0("gzip /3_merge_samples/DUP/outputs/chr_",chr,
