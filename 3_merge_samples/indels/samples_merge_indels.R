...
while(n_row==0 & sample_initial<num_samples){#2
  ...
  sample1 = fread(paste0(path_inputs,"/",
                         ids[sample_initial],"/",ids[sample_initial],"_indels_chr_",chr),
                  header = T)
...
if(sample_initial==num_samples){write.table(region_out[batch_job,],paste0("./empty_batches/chr_",chr,"_start_",start,"_end_",end),row.names=F,quote=F)
}
...
if(sample_initial<num_samples){
  ...
  for(sample_merge in sample_initial:real_num_samples){
    ...
    sample1 = fread(paste0(path_inputs,"/",
                           ids[sample_merge],"/",ids[sample_merge],"_indels_chr_",chr),header = T)
    ...
    if(n_row==0){
      ...
      colnames(my_samples)[(ncol(my_samples)-13):ncol(my_samples)] = paste0(c("chr_", # 13 per N-1
                                                                              "position_",
                                                                              "REF_",
                                                                              "ALT_",
                                                                              "GT_",
  ...
  if(nrow(genos)==1){
    ...
    genos[which(!(genos %in% c("./.","0/1","1/1", "2/1","1/2")))[-1]] = "0/0"
  ...
  if(nrow(genos)>1){
    ...
    genos[!genos %in% c("./.","0/1","1/1","2/1","1/2")] = ""
  ...
  final_merge %>% 
    fwrite(paste0("./outputs/chr_",chr,
                  "/indels_chr_",chr,"_",as.character(start),"_",as.character(end)),row.names=F,sep=" ",na = "NA")
  
  print("Now compress please")
  
  system(paste0("gzip ./outputs/chr_",chr,
              "/indels_chr_",chr,"_",as.character(start),"_",as.character(end)))
  
  print("Now generate VCF")
  
  final_merge = fread(paste0("zcat ./outputs/chr_",chr,
                             "/indels_chr_",chr,"_",as.character(start),"_",as.character(end),".gz"))
  ...
  final_merge$SVTYPE = paste0("SVTYPE=INDEL;")
  ...
  ## length indel ###
  length_indel =  paste0("SVLEN=",abs(nchar(my_vcf$REF)-nchar(my_vcf$ALT)),";")
  ...
  fwrite(my_vcf,paste0("./outputs/chr_",chr,
                       "/indels_chr_",chr,"_",as.character(start),"_",as.character(end),".vcf"),row.names=F,sep="\t")
  
  print("Now compress again please")
  
  system(paste0("cat ./header ./outputs/chr_",chr,
                "/indels_chr_",chr,"_",as.character(start),"_",as.character(end),".vcf > ./outputs/chr_",chr,
                "/indels_chr_",chr,"_",as.character(start),"_",as.character(end),".vcf1" ))
  
  system(paste0("mv ./outputs/chr_",chr,
                "/indels_chr_",chr,"_",as.character(start),"_",as.character(end),".vcf1 ./outputs/chr_",chr,
                "/indels_chr_",chr,"_",as.character(start),"_",as.character(end),".vcf" ))
